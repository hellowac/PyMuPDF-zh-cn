...
.. tab:: 中文
    ...
    本文记录了测量 :title:`PyMuPDF` 性能的方法，以及用于比较的工具和示例文件。

    以下三个部分涉及不同的性能方面：

    * :ref:`文档复制<app4_copying>` - 包括打开和解析 :title:`PDF`，然后将其写入输出文件。由于相同的基本操作也用于合并（连接） :title:`PDF`，因此这些结果同样适用于这些用例。
    * :ref:`文本提取<app4_text_extraction>` - 从 :title:`PDF` 中提取纯文本，并将其写入输出文本文件。
    * :ref:`页面渲染<app4_page_rendering>` - 将 |PDF| 页面转换为与页面完全相同的图像文件。这一功能是使用 :title:`Python GUI` 脚本浏览文档的基本前提。我们选择了中等质量（150 DPI）的版本。
...
.. tab:: 中文
    ...
    所有工具要么是平台独立的，要么至少可以在 :title:`Windows` 和 :title:`Unix` / :title:`Linux` 上运行。
    ...
    .. list-table::
      ...
      * - |PyMuPDF|
        - 本手册中的工具。
      * - PDFrw_
        - 一个纯 :title:`Python` 工具，供 :title:`rst2pdf` 使用，并具有与 :title:`ReportLab` 的接口。
      * - PyPDF2_
        - 一个纯 :title:`Python` 工具，功能广泛。
      * - PDFMiner_
        - 一个纯 :title:`Python` 工具，用于从 |PDF| 中提取文本和其他数据。
      * - XPDF_
        - 一个具有多种功能的命令行实用程序。
      * - PikePDF_
        - 一个 :title:`Python` 包，类似于 :title:`PDFrw`，但基于 :title:`C++` 库 :title:`QPDF`。
      * - PDF2JPG_
        - 一个专门用于将 |PDF| 页面渲染为 :title:`JPG` 图像的 :title:`Python` 包。
...
.. tab:: 中文
    ...
    处理 |PDF| 文件读取及其内容解析的速度如何？由于批量工具总是一次性完全执行请求的任务，从头到尾，因此无法直接比较纯粹的解析性能。 :title:`PDFrw` 也有一种 *惰性* 解析策略，这意味着它只解析当前需要的文档部分。

    为了回答这个问题，我们测量了使用每个工具将 |PDF| 文件复制到输出文件的时间，并不执行其他任何操作。

    以下是每个工具使用的 :title:`Python` 命令：
...
.. tab:: 中文
    ...
    我们测试了 |PyMuPDF| 在与 :title:`pdf2jpg` 和 :title:`XPDF` 的渲染速度对比，分辨率为 150 DPI。

    以下是如何使用每个工具的 :title:`Python` 命令：
